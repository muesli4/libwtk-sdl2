...
slider::slider(int start, int end, int step)
    : _start(start)
    , _end(end)
    , _step(step)
    , _num_steps(std::abs(_end - _start) / std::abs(step))
    , _current_step(0)
    , _pressed(false)
{
}
...
void slider::on_mouse_move_event(mouse_move_event const & e)
{
    ...
    if (_pressed)
    {
        ...
        // TODO call event handler
        // TODO signal on slide or after button up?
